...
if pdf_file is not None:
    try:
        ...
        if page_index >= len(reader.pages):
            ...
            # Helper para extrair número (aceita separadores ., e ,)
            def extract_number(pattern):
                m = re.search(pattern, text, re.IGNORECASE)
                if not m:
                    return 0
                num = m.group(1)
                # normaliza separadores (remove pontos e vírgulas)
            ...
            # Padrões comuns (plural/singular, diferentes formas)
            homens_count = extract_number(r"([\d\.,]+)\s+homens?")
            mulheres_count = extract_number(r"([\d\.,]+)\s+mulheres?")
            # padrão para "advogados com filhos" ou apenas "com filhos"
            ...
            for i, v in enumerate(values):
                ax.text(i, v + max(values) * 0.01 if max(values) > 0 else v + 0.1, str(v), ha='center')
            st.pyplot(fig)
else:
